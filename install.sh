...
# Safe Zsh check (won't crash in bash with set -u)
...
# Final step: replace current shell with bash
echo ""
...
if ! exec /bin/bash -l 2>/dev/null; then
    ...
    echo "Most likely cause: syntax error in /root/.bashrc (around line 117)"
    echo "Edit the file: nano /root/.bashrc"
    echo "Fix the if/fi mismatch, save, then run 'z-off' again."
    exit 1
fi
INNER
